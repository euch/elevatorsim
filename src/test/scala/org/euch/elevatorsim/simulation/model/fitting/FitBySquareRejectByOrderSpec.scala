...
import org.euch.elevatorsim.simulation.fitting.space.FitBySquareRejectByOrder
...
class FitBySquareRejectByOrderSpec extends AnyFlatSpec with Matchers {
  ...
  private val groupB = LoadGroup("GroupB", "1", List(Passenger(name = "John", dimensions = DimensionsBox(40, 60, 180), weight = 100)))
  private val groupC = LoadGroup("GroupC", "1", List(Passenger(name = "James", dimensions = DimensionsBox(40, 60, 180), weight = 100)))
  new FitBySquareRejectByOrder().fit(cabin, List(groupA, groupB, groupC)) shouldBe
