...
trait WeightFittingRules extends FittingRules {
  ...
  protected def findSplitIdx(weightLimit: Double, transport: Transport, newLoadSorted: List[LoadGroup]): Option[Int] = {
    val usedWeight = transport.totalWeight
    var requiredWeight = usedWeight
    newLoadSorted.map(_.weightSum).zipWithIndex.foreach {
      case (weight, i) =>
