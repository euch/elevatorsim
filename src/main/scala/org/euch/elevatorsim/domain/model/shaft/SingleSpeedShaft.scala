...
case class BaseShaft(override val portals: List[Portal],
                       override val name: String,
                       override val dimensions: DimensionsBox,
                       override val transport: Transport,
                       override val winch: Winch) extends Shaft {
  override val safetyRecords: List[SafetyRecord] = portals.flatMap(_.safetyRecords)
}
